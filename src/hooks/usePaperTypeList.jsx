...
const usePaperTypeList = (
  ...
  const fetchData = async () => {
    try {
      ...
      const { data } = await axiosInstance.get(`${BASE_URL}/paper-types/paginate`, {
        params: {
  ...
  const handleDeletion = async (item, itemName) => {
      ...
      if (isNameMatched) {
        try {
          ...
          const { data } = await axiosInstance.delete(`${BASE_URL}/paper-types/${item.id}`);
...
export default usePaperTypeList;